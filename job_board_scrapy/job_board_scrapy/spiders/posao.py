...
class PosaoSpider(scrapy.Spider):
    ...
    def parse_start_url(self, response):
        info = {
            "zupanije": response.selector.xpath(
                '//*[@class="districts"]/ul/li/a/text()'
            ).extract(),
            "link_zup": response.selector.xpath(
                '//*[@class="districts"]/ul/li/a/@href'
            ).extract(),
        }
    ...
    def parse(self, response, zupanije, first_page):
        ...
        for special_job in response.selector.xpath('//div[@class="intro"]'):
            ...
            try:
                sj_vrijedi_do_posao = datetime.date.today() + datetime.timedelta(
                    days=int((dani_do_isteka.split()[0]))
                )
            except:
                pass
        ...
        if first_page == True:
            j_tvrtke = (
                response.selector.xpath(
                    '//table//div[@class="employer res_date_time_comp"]/text()'
                ).extract()
                + response.selector.xpath(
                    '//table//div[@class="company employer res_date_time_comp"]/text()'
                ).extract()
            )
            j_pozicije = (
                response.selector.xpath('//table//div[@class="title"]/text()').extract()
                + response.selector.xpath(
                    '//table//div[@class="normal_title title"]/text()'
                ).extract()
            )
        else:
            j_tvrtke = (
        ...
        j_mjesta = response.selector.xpath(
            '//table//div[@class="location res_date_time_comp"]/text()'
        ).extract()
        j_datumi = response.selector.xpath(
            '//table//div[@class="deadline res_date_time_comp"]/text()'
        ).extract()
        j_linkovi_za_poslove = response.selector.xpath("//table//a/@href").extract()

        for j_mjesto, j_tvrtka, j_pozicija, j_datum, j_link in zip(
            j_mjesta, j_tvrtke, j_pozicije, j_datumi, j_linkovi_za_poslove
        ):
            try:
                j_datum = datetime.date.today() + datetime.timedelta(
                    days=int((j_datum.split()[0]))
                )
            except:
                pass
        ...
        yield Request(
            url=next_page[-2],
            callback=self.parse,
            cb_kwargs=dict(zupanije=zupanije, first_page=False),
        )
